...
int main(int argc, char** argv) {
	...
	auto cool = Shared<Map<size_t, Shared<String>>>::make(50);

	for (size_t i = 0; i < 50; ++i) {

        *cool += { i, i % 2 == 0 ? Shared<String>::make("Value") : nullptr };
		
	}

	Weak<MutableArray<wchar_t>> weakStr = (*cool)[0].makeWeak();
	(*weakStr)[3] = L'\0';
	
	for (auto const& e : *cool) {

		std::cout << e.getKey() << " | " << (e.getValue().isValid() ? e.getValue().getClass() : e.getValue().getClass()) << std::endl;
        
	}

    String cool2 = "abcabcabcccc";
    auto arr = cool2.split("ac");
